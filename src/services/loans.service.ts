...
import { parseDate, toDateString, getDueDate, today } from '../utils/date.js';
...
export async function getPaymentStatus(tenantId: string, loanId: string) {
  ...
  if (loan.loanType !== 'MONTHLY') {
    throw AppError.badRequest('Payment status is only available for MONTHLY loans');
  }
...
export async function closeLoan(tenantId: string, loanId: string, closedById: string) {
  ...
  // Validate remaining principal is 0
  ...
  const paymentStatus = await getPaymentStatus(tenantId, loanId);
  ...
  // Re-fetch to return updated loan
